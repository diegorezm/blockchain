...
import { env } from "@/env";
import { signOutUseCase } from "@/server/modules/auth/use-cases/sign-out";
import { validateSessionTokenUseCase } from "@/server/modules/auth/use-cases/validate-session";
import { findUserByIdUseCase } from "@/server/modules/user/use-cases/find-by-id";
...
export const signOutAction = async () => {
  ...
  await signOutUseCase(response.session.id);
  const cookieStore = await cookies();
